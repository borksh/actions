...
echo 'GNUPGHOME="'"$GNUPGHOME"'" '"$GPG_EXEC"' '"$PINENTRY_MODE"' --passphrase "$BORK_GPG_PASSPHRASE" --batch --no-tty "$@"' > $GPG_WITH_PASSPHRASE
...
GPG_KEY_ID=$(gpg --list-keys --with-colons | sed -ne "/^pub:/ p;" | cut -d: -f5)
